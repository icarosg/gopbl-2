...
var servidores = []string{
	"http://172.16.201.2:8083", //22
	"http://172.16.201.2:8085", //shell
	"http://172.16.201.2:8084", //ipiranga
}
...
func main() {
	...
	var erro error
	dbServer, erro = db.NovaConexaoDB(nomeServidor, "172.16.201.2", portaDB)
	if erro != nil {
...
func handleListarPostos(client mqtt.Client, msg mqtt.Message) {
	...
	for _, servidor := range servidores {
		if servidor == "http://172.16.103.11:8083" {
			continue
		}
