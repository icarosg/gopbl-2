...
var messagePubHandler MQTT.MessageHandler = func(client MQTT.Client, msg MQTT.Message) {
	...
	switch msg.Topic() {
	case "topic/pedido-consulta-cliente-1-B":
		token := client.Publish("topic/listar-postos-2", 0, false, "listar postos")
		token.Wait()
		...
		// 	return
		// }
		// token = mqttClient.Publish("topic/consulta-cliente-1-B", 0, false, payload)
		// token.Wait()
	...
	case "topic/receber-posto":
		...
		token := mqttClient.Publish("topic/consulta-cliente-1-B", 0, false, payload)
		token.Wait()
		fmt.Println("postos enviados para o cliente 1-B")
		client.Publish("topic/confirmacao-veiculo-2", 0, false, "")
		//postosChanA <- posto
		fmt.Println("recebi o posto")
	case "topic/reqAtomica-2":
...
func subscribeToTopics() {
	...
	topic = "topic/pedido-consulta-cliente-1-B"
	token = mqttClient.Subscribe(topic, 1, nil)
	token.Wait()
	fmt.Printf("Subscribed to topic: %s\n", topic)

	topic = "topic/receber-posto-2"
	token = mqttClient.Subscribe(topic, 1, nil)
	token.Wait()
	fmt.Printf("Subscribed to topic: %s\n", topic)

	topic = "topic/reqAtomica-2"
	token = mqttClient.Subscribe(topic, 1, nil)
	token.Wait()
	fmt.Printf("Subscribed to topic: %s\n", topic)

	topic = "topic/cadastro-posto-2"
	token = mqttClient.Subscribe(topic, 1, nil)
	token.Wait()
	fmt.Printf("Subscribed to topic: %s\n", topic)

	topic = "topic/encerrar-viagem-2"
	token = mqttClient.Subscribe(topic, 1, nil)
	token.Wait()
...
func setupMQTT() {
	...
	opts.SetClientID("go_mqtt_server_2")
