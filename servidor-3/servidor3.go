...
var messagePubHandler MQTT.MessageHandler = func(client MQTT.Client, msg MQTT.Message) {
	...
	switch msg.Topic() {
	case "topic/pedido-consulta-cliente-1-C":
		token := client.Publish("topic/listar-postos-3", 0, false, "listar postos")
		token.Wait()
		...
		// 	return
		// }
		// token = mqttClient.Publish("topic/consulta-cliente-1-C", 0, false, payload)
		// token.Wait()
		// fmt.Println("postos enviados para o cliente 1-C")
	case "topic/receber-posto-3":
		...
		token.Wait()
		fmt.Println("postos enviados para o cliente 1-C")
		client.Publish("topic/confirmacao-veiculo-3", 0, false, "")
		//postosChanA <- posto
		fmt.Println("recebi o posto")
	case "topic/reqAtomica-3":
		...
		// token.Wait()
	case "topic/possivel-reserva-server-3":
		var confirm bool
...
func main() {
	...
	portaDB := 27017
	nomeServidor := "Estado-Pernambuco"
...
func subscribeToTopics() {
	...
	topic = "topic/pedido-consulta-cliente-1-C"
	token = mqttClient.Subscribe(topic, 1, nil)
	token.Wait()
	fmt.Printf("Subscribed to topic: %s\n", topic)

	topic = "topic/receber-posto-3"
	token = mqttClient.Subscribe(topic, 1, nil)
	token.Wait()
	fmt.Printf("Subscribed to topic: %s\n", topic)

	topic = "topic/reqAtomica-3"
	token = mqttClient.Subscribe(topic, 1, nil)
	token.Wait()
	fmt.Printf("Subscribed to topic: %s\n", topic)

	topic = "topic/cadastro-posto-3"
	token = mqttClient.Subscribe(topic, 1, nil)
	token.Wait()
	fmt.Printf("Subscribed to topic: %s\n", topic)

	topic = "topic/encerrar-viagem-3"
	token = mqttClient.Subscribe(topic, 1, nil)
	token.Wait()
...
func setupMQTT() {
	...
	opts.SetClientID("go_mqtt_server_3")
